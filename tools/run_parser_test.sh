...
# Usage:
#   ./tools/run_parser_test.sh <input.sy> [expected.parser] [out.parser]
# If expected.parser is omitted, the script will look for testcase/parser/<basename>.parser
...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <input.sy> [expected.parser] [out.parser]"
    exit 2
fi
...
if [ -z "$EXPECTED" ]; then
    EXPECTED="$ROOT_DIR/testcase/parser/$(basename "$INPUT" .sy).parser"
fi

if [ -z "$OUT" ]; then
    OUT="$ROOT_DIR/test_output/$(basename "$INPUT" .sy).parser"
fi
...
echo "Running parser on: $INPUT"
"$BIN" -parser -o "$OUT" "$INPUT"

echo "Generated: $OUT"
if [ ! -f "$EXPECTED" ]; then
    echo "Expected file not found: $EXPECTED"
    echo "You can inspect $OUT for parser output." 
    exit 4
fi
